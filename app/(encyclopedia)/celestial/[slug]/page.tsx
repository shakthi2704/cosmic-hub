...
// ─── Fast Facts Panel (exoplanets) ─────────────────────────

function classifyExoSize(radiusEarthRadii: number | null | undefined): string | null {
    if (radiusEarthRadii == null) return null
    if (radiusEarthRadii < 0.5) return 'Sub-Earth'
    if (radiusEarthRadii < 1.25) return 'Earth-sized'
    if (radiusEarthRadii < 2.0) return 'Super-Earth'
    if (radiusEarthRadii < 6.0) return 'Sub-Neptune'
    if (radiusEarthRadii < 15.0) return 'Neptune-sized'
    return 'Gas Giant'
}

function habitabilityBadge(eqtK: number | null | undefined): { label: string; color: string } | null {
    if (eqtK == null) return null
    if (eqtK >= 200 && eqtK <= 320) return { label: 'Temperate zone', color: '#4ade80' }
    if (eqtK < 200) return { label: 'Likely frozen', color: '#60a5fa' }
    return { label: 'Likely scorched', color: '#f87171' }
}

function FastFactsPanel({ attrs, accent }: { attrs: Record<string, unknown>; accent: string }) {
    const radius = attrs.radiusEarthRadii as number | null | undefined
    const eqt = attrs.equilibriumTempK as number | null | undefined
    const distPc = attrs.distanceFromEarthParsecs as number | null | undefined
    const sizeClass = classifyExoSize(radius)
    const habitability = habitabilityBadge(eqt)
    const ly = distPc != null ? distPc * 3.26156 : null

    if (!sizeClass && !habitability && ly == null) return null
    ...
    return (
        <div className="flex flex-wrap gap-2.5">
            ...
            {habitability && (
                <span
                    className="text-xs font-medium px-3 py-1.5 rounded-full border"
                    style={{ color: habitability.color, borderColor: `${habitability.color}40`, backgroundColor: `${habitability.color}12` }}
                >
                    {habitability.label}
                </span>
            )}
            {ly != null && (
                <span className="text-xs font-medium px-3 py-1.5 rounded-full border border-white/15 text-white/60 bg-white/[0.03]">
                    {ly.toFixed(1)} light-years away
                </span>
            )}
        </div>
    )
            ...
            <div className="max-w-5xl mx-auto px-6 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {/* Fast Facts (exoplanets only) */}
                        {obj.type === 'EXOPLANET' && <FastFactsPanel attrs={attrs} accent={meta.accent} />}
